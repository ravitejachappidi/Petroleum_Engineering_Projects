...
if uploaded_file is not None:
    ...
    porosity = st.sidebar.number_input("Porosity of Formation(fraction)")
    height = st.sidebar.number_input("Height of Formation(ft)")
    ct = st.sidebar.number_input("Compressbility(in psi^-1 * 10e-6)")
    flowRate = st.sidebar.number_input("Flow Rate of fluid(RB/D))")
    fvf_o = st.sidebar.number_input("Formation volume factor of Oil(Bo)")
    rw = st.sidebar.number_input("Borehole radius(ft)")
    viscosity = st.sidebar.number_input("Viscosity of fluid(mu)")
    pwf = st.sidebar.number_input("Bottom Hole Pressure Pwf(psia)")
